...
import icbm.classic.prefab.item.IcbmBlockItem;
...
public class ItemBlockExplosive extends IcbmBlockItem {
    ...
    @Override
    public void getDetailedInfo(ItemStack stack, @Nullable Player player, List list) {
        final ExplosiveType data = ICBMClassicHelpers.getExplosive(stack.getItemDamage(), true);
        if (data != null) {
            final WeaponTier tierdata = data.getTier();
            if (tierdata != WeaponTier.NONE) // only show the tier if its not NONE. Tier NONE is currently only assinged to the Missile Module
            {
                list.add(TextFormatting.DARK_RED + LanguageUtility.getLocal("info.misc.tier") + ": " + tierdata.getTooltipStyle() + tierdata.getLocalizedName());
            }
    ...
    protected boolean shouldTauntPlayer(@Nullable Player player) {
