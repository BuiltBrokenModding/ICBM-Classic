...
import icbm.classic.world.missile.logic.flight.ArcFlightLogic;
import icbm.classic.world.missile.logic.flight.WarmupFlightLogic;
...
public class LauncherCapability extends LauncherBaseCapability {
    ...
    private static final Vec3[] SPAWN_OFFSETS = new Vec3[]{
        //DOWN
        new Vec3(0, -3.1f, 0),
        //UP
        new Vec3(0, 2.6f, 0),
        //NORTH
        new Vec3(0, -0.2f, -2.8f),
        //SOUTH
        new Vec3(0, -0.2f, 2.8f),
        //WEST
        new Vec3(-2.8f, -0.2f, 0),
        //EAST
        new Vec3(2.8f, -0.2f, 0)
    };
    private final LauncherBaseBlockEntity host;
    ...
    public IActionStatus launch(ILauncherSolution solution, @Nullable IMissileCause cause, boolean simulate) {
        ...
        final BlockCause selfCause = new BlockCause(host.getLevel(), host.getPos(), host.getBlockState()); // TODO add more information about launcher
        selfCause.setPreviousCause(cause);

        final Vec3 spawnPosition = SPAWN_OFFSETS[host.getLaunchDirection().ordinal()].addVector(host.getPos().getX() + 0.5, host.getPos().getY() + 0.5, host.getPos().getZ() + 0.5);
        final MissileSource source = new MissileSource(host.getLevel(), spawnPosition, selfCause);
        ...
        if (stack.hasCapability(ICBMClassicAPI.MISSILE_STACK_CAPABILITY, null)) {
            ...
            if (missileStack != null) {
                ...
                final Vec3 target = applyInaccuracy(targetData.getPosition(), Math.max(1, solution.getFiringCount()));
                ...
                final IMissile missile = missileStack.newMissile(host.getLevel());
    ...
    private IActionStatus fireMissile(IMissile missile, MissileSource source, Vec3 target) {
        ...
        entity.getXRot() = entity.prevRotationPitch = host.getMissilePitch(false);
        entity.getYRot() = entity.prevRotationYaw = host.getMissileYaw(false);
        ...
        entity.setPos(source.getPosition().x, source.getPosition().y, source.getPosition().z);
    ...
    @Override
    public float getInaccuracy(Vec3 target, int launcherCount) {
    ...
    protected Vec3 applyInaccuracy(Vec3 target, int launcherCount) {

        //Randomize distance
        float inaccuracy = getInaccuracy(target, launcherCount) * host.getLevel().rand.nextFloat();

        //Randomize radius drop
        angle.setYaw(host.getLevel().rand.nextFloat() * 360); //TODO fix to use a normal distribution from ICBM 2

        //Apply inaccuracy to target position and return
        return new Vec3(target.x + angle.x() * inaccuracy, 0, target.z + angle.z() * inaccuracy);
    }
     ...
     * @return
     */
    public boolean isTargetTooClose(Vec3 target) {
    ...
    public boolean isTargetTooFar(Vec3 target) {
        final double deltaX = Math.abs(target.x - (host.getPos().getX() + 0.5));
        final double deltaZ = Math.abs(target.z - (host.getPos().getZ() + 0.5));
