...
import net.neoforged.common.ForgeChunkManager;
 ...
 */
public class MissileTrackerLevel extends WorldSavedData {
    //Constants
    ...
    public MissileTrackerLevel(String identifier) {
        super(identifier);
     ...
     * @param missile
     */
    void simulateMissile(ExplosiveMissileEntity missile) {
        ...
        //Only run on server
        if (!missile.world.isClientSide() && missile.getMissileCapability().getTargetData() != null) {
            ...
            //Calculate distance
            double dx = missile.getMissileCapability().getTargetData().getX() - missile.getX();
            double dz = missile.getMissileCapability().getTargetData().getZ() - missile.getZ();
     ...
     */
    public void onWorldTick(final Level level) {
    ...
    private void Launch(final Level level, MissileTrackerData mtd) {
        //Create entity
        ExplosiveMissileEntity missile = new ExplosiveMissileEntity(world);

        //Set data
        missile.readEntityFromNBT(mtd.missileData);
        missile.getY() = 250; //TODO pull from config
        missile.getX() = mtd.targetPos.x(); //TODO calculate arc position at 250 so we don't come in on top of the target
        missile.getZ() = mtd.targetPos.z();
    ...
    public void readFromNBT(CompoundTag nbt) {
        //Load missiles
        ListTag list = nbt.getTagList(NBTConstants.MISSILES, 10);
        for (int i = 0; i < list.size(); i++) {
            CompoundTag missileSave = list.getCompoundTagAt(i);
