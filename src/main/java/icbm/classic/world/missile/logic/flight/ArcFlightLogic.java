...
public class ArcFlightLogic implements IMissileFlightLogic {
    public static final ResourceLocation REG_NAME = new ResourceLocation(IcbmConstants.MOD_ID, "path.arc");
    ...
    @Override
    public void calculateFlightPath(final Level level, double startX, double startY, double startZ, final IMissileTarget targetData) {
    ...
    protected void runFlightLogic(Entity entity, int ticksInAir) {
        ...
        if (!entity.world.isClientSide()) {
            ...
            if (Math.abs(entity.getX() - endX) <= 0.1f && Math.abs(entity.getZ() - endZ) <= 0.1f) {
                entity.motionX = 0;
                entity.motionZ = 0;
            }
            ...
            // Sim system
            if (entity instanceof ExplosiveMissileEntity && shouldSimulate(entity)) {
                MissileTrackerHandler.simulateMissile((ExplosiveMissileEntity) entity); //TODO add ability to simulate any entity
            }
        }
    }
    ...
    protected void alignWithMotion(Entity entity) {
        entity.getXRot() = (float) (Math.atan(entity.motionY / (Math.sqrt(entity.motionX * entity.motionX + entity.motionZ * entity.motionZ))) * 180 / Math.PI);
        // Look at the next point
        entity.getYRot() = (float) (Math.atan2(entity.motionX, entity.motionZ) * 180 / Math.PI);
    }
    ...
    protected void idleMissileOnPad(Entity entity, int ticksInAir) {
        entity.getXRot() = entity.prevRotationPitch = 90;
    ...
    protected boolean shouldSimulate(Entity entity) {
        if (EntityMissile.hasPlayerRiding(entity)) {
            return false;
        } else if (entity.getY() >= ConfigMissile.SIMULATION_START_HEIGHT) {
            return true;
        }
