...
public abstract class BlastBeam extends Blast implements IBlastTickable {
    ...
    protected final Set<FlyingBlockEntity> flyingBlocks = new HashSet();
    protected final List<BlockPos> blocksToRemove = new ArrayList();

    protected LightBeamEntity lightBeam;
    ...
    protected boolean doExplode(int callCount) {
        if (!hasDoneSetup) {
            ...
            this.level().createExplosion(this.exploder, location.x(), location.y(), location.z(), 4F, true);

            //Create beam
            this.lightBeam = new LightBeamEntity(this.level())
            ...
            this.level().spawnEntity(this.lightBeam);
        }
