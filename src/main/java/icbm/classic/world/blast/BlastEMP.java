...
public class BlastEMP extends Blast {
    ...
    public boolean doExplode(int callCount) {
        if (!level().isClientSide()) {
                        ...
                        for (int z = (int) -this.getBlastRadius(); z < (int) this.getBlastRadius(); z++) {
                            ...
                            if (world.isBlockLoaded(blockPos)) {
                                ...
                                if (Math.round(location.x() + y) == location.yi()) {
                                    level().spawnParticle(EnumParticleTypes.SMOKE_LARGE, blockPos.getX() + 0.5, blockPos.getY() + 0.5, blockPos.getZ() + 0.5, 0, 0, 0);
                                }
                                ...
                                if (!MinecraftForge.EVENT_BUS.post(new EmpEvent.BlockPre(this, world, blockPos, iBlockState))) {
                                    if (ICBMClassicHelpers.hasEmpHandler(iBlockState)) {
                                        ...
                                        BlockEntity blockEntityEntity = world.getBlockEntity(blockPos);
                                        if (tileEntity != null) {
            ...
            if (this.effectEntities && ConfigEMP.ALLOW_ENTITY) {
                ...
                List<Entity> entities = level().getEntitiesWithinAABB(Entity.class, bounds);
                    ...
                    if (!MinecraftForge.EVENT_BUS.post(new EmpEvent.EntityPre(this, entity))) {
                        ...
                        if (entity.hasCapability(CapabilityEMP.EMP, null)) {
                            ...
                            if (receiver != null) {
                                ...
                                doInventory = receiver.shouldEmpSubObjects(world, entity.getX(), entity.getY(), entity.getZ());
                            }
            ...
            ICBMSounds.EMP.play(world, location.x(), location.y(), location.z(), 4.0F, (1.0F + (level().rand.nextFloat() - level().rand.nextFloat()) * 0.2F) * 0.7F, true);
        }
        return true;
    }
    ...
    protected float empEntity(Entity entity, float powerEntity, IEMPReceiver receiver) {
        if (receiver != null) {
            powerEntity = receiver.applyEmpAction(world, entity.getX(), entity.getY(), entity.getZ(), this, powerEntity, true);
            //TODO spawn effects on entity if items were effected
            //TODO ICBMClassic.proxy.spawnShock(this.oldLevel(), this.position, new Pos(entity), 20);
        }
        return powerEntity;
    }

    protected float empEntity(BlockEntity entity, float powerEntity, IEMPReceiver receiver) {
        if (receiver != null) {
