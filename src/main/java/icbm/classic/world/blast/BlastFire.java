...
public class BlastFire extends Blast {
    ...
    public boolean doExplode(int callCount) {
                ...
                for (int y = 0; y < radius; ++y) {
                    for (int z = 0; z < radius; ++z) {
                        if (x == 0 || x == radius - 1 || y == 0 || y == radius - 1 || z == 0 || z == radius - 1) {
                            ...
                            float energy = radius * (0.7F + level().rand.nextFloat() * 0.6F);
                            ...
                            for (float stepAmount = 0.3F; energy > 0.0F; energy -= stepAmount * 0.75F) {
                                ...
                                if (!block.isAir(blockState, world, targetPosition)) {
                                    energy -= (block.getExplosionResistance(level(), targetPosition, this.exploder, this) + 0.3F) * stepAmount;
                                }
                                ...
                                if (energy > 0.0F) {
                                    ...
                                    if (chance > distanceFromCenter * 0.55) {
                                        boolean canReplace = block.isReplaceable(level(), targetPosition) || block.isAir(blockState, level(), targetPosition);

                                        if (canReplace && Blocks.FIRE.canPlaceBlockAt(level(), targetPosition)) {
        ...
        ICBMSounds.EXPLOSION_FIRE.play(world, location.x() + 0.5D, location.y() + 0.5D, location.z() + 0.5D, 4.0F, (1.0F + (level().rand.nextFloat() - level().rand.nextFloat()) * 0.2F) * 1F, true);
        return true;
    }
}
