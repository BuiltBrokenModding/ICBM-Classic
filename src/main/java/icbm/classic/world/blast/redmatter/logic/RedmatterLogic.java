...
import icbm.classic.world.blast.helpers.BlastBlockHelpers;
import icbm.classic.world.blast.redmatter.DamageSourceRedmatter;
...
public class RedmatterLogic {
    //Host of the logic
    public final RedmatterEntity host;
     ...
     */
    public RedmatterLogic(RedmatterEntity host) {
        this.host = host;
    }
    ...
    protected void doTick() {
        ...
        if (ConfigBlast.redmatter.ENABLE_AUDIO) {
            ...
            if (host.world.rand.nextInt(8) == 0) {
                final double playX = host.getX() + CalculationHelpers.randFloatRange(host.world.rand, host.getBlastSize());
                final double playY = host.getY() + CalculationHelpers.randFloatRange(host.world.rand, host.getBlastSize());
                final double playZ = host.getZ() + CalculationHelpers.randFloatRange(host.world.rand, host.getBlastSize());
            ...
            ICBMSounds.REDMATTER.play(host.world, host.getX(), host.getY(), host.getZ(), 3.0F, CalculationHelpers.randFloatRange(host.world.rand, -0.8f, 1.2f), true);
        }
    }
    ...
    protected void cycleDestroyBlocks() {
        ...
        final Vec3 center = host.getPositionVector();
     ...
     */
    protected void rayTraceTowardsBlock(final Vec3 center, final BlockPos target) {
        ...
        final Vec3 pos = new Vec3(targetX, targetY, targetZ);
    ...
    protected void doEntityEffects() {
        ...
        final AxisAlignedBB bounds = new AxisAlignedBB(
            host.getX() - entityRadius,
            host.getY() - entityRadius,
            host.getZ() - entityRadius,
            host.getX() + entityRadius,
            host.getY() + entityRadius,
            host.getZ() + entityRadius);
    ...
    private boolean shouldHandleEntity(Entity entity) {
        ...
        if (entity instanceof Player && (((Player) entity).capabilities.isCreativeMode || ((Player) entity).capabilities.disableDamage)) {
            return false;
        }
    ...
    protected void handleEntities(Entity entity) {
        ...
        final double xDifference = host.getX() - entity.getX();
        final double yDifference = host.getY() - entity.getY();
        final double zDifference = host.getZ() - entity.getZ();
    ...
    private void attackEntity(Entity entity, double distance) {
        ...
        if (distance < attackRange) {
            if (entity instanceof RedmatterEntity && !entity.isDead) {
                ...
                final double targetRad = Math.pow(((RedmatterEntity) entity).getBlastSize(), 3);
                ...
                final IBlast blast = ((ExplosionEntity) entity).getBlast();
                ...
                ExplosiveHandler.createExplosion(host, entity.world, entity.getX(), entity.getY(), entity.getZ(), explosive);
                entity.setDead();
            } else if (entity instanceof EntityLiving || entity instanceof Player) {
                entity.attackEntityFrom(new DamageSourceRedmatter(this), 2000);
            } else {
                //Kill entity in the center of the ball
                entity.setDead();
                if (entity instanceof FlyingBlockEntity) {
