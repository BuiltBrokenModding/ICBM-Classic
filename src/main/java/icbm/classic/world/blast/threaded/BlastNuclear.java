...
import icbm.classic.world.blast.BlastMutation;
import icbm.classic.world.blast.BlastRot;
...
public class BlastNuclear extends BlastThreaded {
    ...
    public boolean setupBlast() {
        ...
        if (this.level() != null) {
                    ...
                    for (int z = -r; z < r; z++) {
                        ...
                        if (r > distance && r - 3 < distance) {
                            ...
                            world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, spawnPosition.x(), spawnPosition.y(), spawnPosition.z(),
                                xDiff * 0.3 * level().rand.nextFloat(), -level().rand.nextFloat(), zDiff * 0.3 * level().rand.nextFloat()); //(float) (distance / this.getRadius()) * oldLevel().rand.nextFloat(), 0, //0, 8F, 1.2F);
                        }
                    }
                }
            }
            ...
            ICBMSounds.EXPLOSION.play(world, this.location.x(), this.location.y(), this.location.z(), 7.0F, (1.0F + (this.level().rand.nextFloat() - this.level().rand.nextFloat()) * 0.2F) * 0.7F, true);
        }
            ...
            for (int x = -r; x < r; x++) {
                for (int z = -r; z < r; z++) {
                    ...
                    if (distance < r && distance > r - 1) {
                        ...
                        if (this.level().rand.nextFloat() < Math.max(0.001 * r, 0.05)) {
    ...
    public void onBlastCompleted() {
        ...
        if (level() != null && !level().isClientSide()) {
            try {
                ...
                //Play audio
                ICBMSounds.EXPLOSION.play(world, this.location.x(), this.location.y(), this.location.z(), 10.0F, (1.0F + (this.level().rand.nextFloat() - this.level().rand.nextFloat()) * 0.2F) * 0.7F, true);
