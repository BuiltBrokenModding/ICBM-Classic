...
public class BlastAntimatter extends BlastThreaded {
    ...
    public boolean setupBlast() {
        super.setupBlast();
        ICBMSounds.ANTIMATTER.play(world, this.location.x(), this.location.y(), this.location.z(), 7F, (float) (this.level().rand.nextFloat() * 0.1 + 0.9F), true);
    ...
    protected boolean shouldEditPos(int x, int y, int z) {
        ...
        if (delta < featherEdge) {
            ...
            return level().rand.nextFloat() > p2;
        }
        return true;
    }
    ...
    protected boolean doDamageEntities(float radius, float power, boolean destroyItem) {
        ...
        if (!ConfigBlast.ANTIMATTER_BLOCK_AND_ENT_DAMAGE_ON_REDMATTER) {
            ...
            final long killed = allEntities.stream().filter(e -> e instanceof RedmatterEntity).peek(this::onDamageEntity).count();
            if (killed > 0) {
                return false;
            }
    ...
    protected boolean onDamageEntity(Entity entity) {
        if (entity instanceof RedmatterEntity) {
