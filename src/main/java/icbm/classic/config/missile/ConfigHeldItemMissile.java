 ...
 */
public class ConfigHeldItemMissile
{
    @Config.Name("item_ban_allow") // TODO rename to `items`
    @Config.Comment("Items allowed for insertion into held-item missile as a projectile payload")
    public BanList BAN_ALLOW = new BanList();

    @Config.Name("block_damage_multiplier")
    @Config.Comment("Multiplier to apply to missile's held item's block break speed.")
    public float BLOCK_DAMAGE_MULTIPLIER = 4;

    public static class BanList {

        @Config.Name("ban") //TODO rename to `invert`
